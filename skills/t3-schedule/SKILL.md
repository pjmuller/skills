...
```bash
t3-schedule add --name fleet-report-dentai --at 07:30 --weekdays \
  --project ~/code/rootcause-org/rootcause-brain-dentai --model fable \
  -- "Run the fleet-report-dentai skill for yesterday, end to end (ClickUp ticket included)."
t3-schedule pick-profile    # Claude profile with the most spare room now (pjroute rule, scripted)
t3-schedule list            # jobs · loaded? · profile/model · last run + result
t3-schedule list --markdown # same as a table — use this when PJ asks "what's scheduled?"
...
[fire-and-forget flow](../../../ai/skills/t3-manage-thread/spawn-thread.md#fire-and-forget---settle-when-done),
...
One job = LaunchAgent `com.pjmuller.t3-schedule.<name>` (`~/Library/LaunchAgents`, `ProcessType=Interactive` —
`Standard` still clamps the t3 CLI to 15-25 s per call, measured 2026-09-09) → runner
`~/.t3/userdata/scheduled/t3-schedule/<name>.sh` → waits for T3 Code (≤ `--wait-max` min, default 120)
...
Why this shape and what it works around: `decissions/t3_monkey_patches.md` (registry, upstream
status, removal condition). Install: `scripts/install` (symlink into `~/.local/bin`, needs the
...
- **Profile.** Omit `--profile` → the runner asks `t3-schedule pick-profile` at fire time: pjroute's
  step 3 made deterministic (skip session ≥90 % / weekly ≥95 % used; rank by long-window room + ½
...
- **Spacing.** Slots are ≥15 min apart (05:00 hello · 05:15 DA · 05:30 KA · 05:45 hygiene) and the
...
- **Missed or failed slot ⇒ catch-up, not tomorrow.** One extra LaunchAgent
  `com.pjmuller.t3-schedule.catch-up` (every 15 min + at login, armed by `add`/`refresh`) kickstarts
  ...
  written on success only). 2026-09-09 every 05:00 job failed for 90 min (T3 rejected fresh CLI
  sessions with 401 until its 06:27 self-update) and was silently lost — hence this poller.
...
- **Provider limits / auth** are the thread's problem, same as a manual spawn: a rate-limited
  ...
  thread. The daily notification + sidebar thread `⏰ name date` is the ack; nothing else pings PJ.
