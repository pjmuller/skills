...
reuse `_credentials()` and refresh with `google.auth.transport.requests.Request`; never print tokens.
...
- Keep the draft ID. Read raw MIME so recipients, subject, reply headers, attachments, and
