...
3. Repeated speech-to-text misspellings of names are glossary candidates — extend an existing speech-to-text/glossary rule, never duplicate it.
4. Repeats confined to one project belong in that repo's AGENTS.md, not globally.
5. If an instruction file already covers a pattern, report `already covered, being ignored?` instead of proposing duplicate wording.
...
- **Proposed wording:** one terse line in the target file's style, or `already covered, being ignored?`.
