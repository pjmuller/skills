...
| [agents-md](skills/agents-md/SKILL.md) | create or simplify a repo's AGENTS.md / CLAUDE.md, write, prune and merge the global cross-project prompt ([template](skills/agents-md/global-template.md)), or write a handoff ([patterns](skills/agents-md/handoff.md)) | global | `pnpm dlx skills add pjmuller/skills -s agents-md -g -y` (no command installer) |
| [tone-of-voice](skills/tone-of-voice/SKILL.md) | an agent drafts in someone's name and no tone-of-voice file exists: interview + real samples → `tone-of-voice.md` ([starter](skills/tone-of-voice/template.md)) | global | `pnpm dlx skills add pjmuller/skills -s tone-of-voice -g -y` (no command installer) |
...
| [video-shrink-for-gemini](skills/video-shrink-for-gemini/SKILL.md) | a screen recording (or Fathom/Leexi call) must be small enough for Gemini and come with a context-rich prompt | setup repo | `pnpm dlx skills add pjmuller/skills -s video-shrink-for-gemini -y` |
